...
document.addEventListener("deviceready", onload, false);
    ...
    function onload() { 
        ...
        initial3.addEventListener('tap', () => {
            db = window.sqlitePlugin.openDatabase(
                {name: "aacdatabase_NL.db", location: 'default', createFromLocation: 1});
                db.transaction(function(tx) {
                    tx.executeSql('SELECT * FROM maintable', [], function(tx, rs) {
                        var len = rs.rows.length;
                        console.log(len + " rows found.");
        ...
        initial4.addEventListener('tap', () => {
            db = window.sqlitePlugin.openDatabase(
                {name: "aacdatabase_DK.db", location: 'default', createFromLocation: 1});
                db.transaction(function(tx) {
                    tx.executeSql('SELECT * FROM maintable', [], function(tx, rs) {
                        var len = rs.rows.length;
                        console.log(len + " rows found.");
